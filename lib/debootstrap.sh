...
debootstrap_ng()
{
	...
	if [[ $BUILD_DESKTOP == yes ]]; then local tmpfs_max_size=5500; else local tmpfs_max_size=3500; fi # MiB
	...
	if [[ -f $cache_fname && "$ROOT_FS_CREATE_ONLY" != "force" ]]; then
		...
		if [[ $NO_APT_CACHER != yes ]]; then
			...
			local apt_mirror="http://${APT_PROXY_ADDR:-localhost:3142}/$APT_MIRROR"
                        ...
                        local apt_extra="-o Acquire::AllowInsecureRepositories=true -o Acquire::AllowDowngradeToInsecureRepositories=true -o APT::Get::AllowUnauthenticated=true"
...
prepare_partitions()
{
	...
	if [[ $(lsb_release -sc) =~ bionic|buster|bullseye|cosmic|eoan|focal|kali-rolling ]]; then
