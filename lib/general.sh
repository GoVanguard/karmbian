...
create_sources_list()
{
	...
	case $release in
	kali-rolling)
	cat <<-EOF > "${basedir}"/etc/apt/source.list
	deb http://${KALI_MIRROR}/kali kali-rolling main non-free contrib
	EOF
	;;
...
# function: cycle trough distributions
	...
	local distributions=("xenial" "stretch" "bionic" "buster" "bullseye" "eoan" "focal" "kali-rolling")
	local errors=0
...
repo-manipulate() {
	local DISTROS=("xenial" "stretch" "bionic" "buster" "bullseye" "eoan" "focal" "kali-rolling")
	case $@ in
		serve)
...
prepare_host()
{
	...
	if [[ -z $codename || "bionic buster eoan focal debbie tricia ulyana kali-rolling" != *"$codename"* ]]; then
